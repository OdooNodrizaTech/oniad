            ...
            if len(answers) > 0:
                for answer in answers:
                    ...
                    if len(surveymonkey_survey_response_question_answer_ids) == 0:
                        ...
                        self.env['surveymonkey.survey.response.question.answer'].sudo().create(vals)
                        ...
                        for response_item in return_api_survey_responses['response']:
                            if 'result' in response_item:
                                if response_item['result']['response_status'] == 'completed':
                                    ...
                                    surveymonkey_survey_response_obj = self.env['surveymonkey.survey.response'].sudo().create(vals)
                                    ...
                                    if len(response_item['result']['custom_variables']) >0 :
                                        for key,val in response_item['result']['custom_variables'].items():
                                            ...
                                            self.env['surveymonkey.survey.response.custom.variable'].sudo().create(vals)
                                    # pages
                                        ...
                                        for page in response_item['result']['pages']:
                                            ...
                                            if surveymonkey_survey_page_ids:
                                                ...
                                                if return_api_survey_page['status_code'] == 200:
                                                    ...
                                                    surveymonkey_survey_page_obj = self.env['surveymonkey.survey.page'].sudo().create(vals)
                                                    surveymonkey_survey_page_id = surveymonkey_survey_page_obj
                                            # questions
                                            for question in page['questions']:
                                                # if need create question
                                                surveymonkey_question_id = False
                                                surveymonkey_question_ids = self.env['surveymonkey.question'].search(
                                                    [
                                                        ('question_id', '=', question['id'])
                                                    ]
                                                )
                                                if surveymonkey_question_ids:
                                                    ...
                                                    if return_api_survey_page_question['status_code'] == 200:
                                                        ...
                                                        # other
                                                        surveymonkey_question_obj = self.env['surveymonkey.question'].sudo().create(vals)
